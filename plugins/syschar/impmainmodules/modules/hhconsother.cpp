#include "hhconsother.h"
#include "ui_hhconsother.h"
#include "delegates/imagecheckdelegate.h"
#include <qsqlquery.h>
#include <QSqlError>
#include <calcdialog.h>
#include "moduleglobal.h"
#include "submodules/hhconsprodslkp.h"
#include <QDebug>

hhconsother::hhconsother(QWidget *parent, QSqlDatabase db, QString systemID) :
    QWidget(parent),
    ui(new Ui::hhconsother)
{
    ui->setupUi(this);
    dataBase = db;
...
hhconsother::~hhconsother()
{
    //qDebug() << "Before destroy hhconsother UI";
    delete ui;
    //qDebug() << "After destroy hhconsother UI";
}

void hhconsother::loadAndroidEffects()
{
...
void hhconsother::loadForm()
{
    ...
    m_colModel->insertField("PROD_DESC","Product","000","000",false,false,true);
    m_colModel->insertField("ENERGY","Energy (MJ / kg)","0","0",false,false,true);
    m_colModel->insertField("PROTEIN","Protein (g / kg)","0","0",false,false,true);
    ...
    m_periodModel->setTableName("otherhhcpperiods");
    m_periodModel->addField("tmp","Consumed in","N","");
     m_periodModel->addField("qtp","Quantity (kg or lt)","0","");
...
void hhconsother::constructCustomDelegator()
{
...
void hhconsother::loadChildData(QModelIndex index)
{
...
void hhconsother::setChildStatus(bool status)
{
...
void hhconsother::setParentStatus(bool status)
{
...
void hhconsother::childDataChanged()
{
...
void hhconsother::on_BitBtn14_clicked()
{
    m_colModel->insertNewItem();
}

void hhconsother::on_BitBtn15_clicked()
{
    QModelIndex idx;
...
void hhconsother::on_BitBtn9_clicked()
{
...
void hhconsother::on_BitBtn10_clicked()
{
...
void hhconsother::on_BitBtn11_clicked()
{
...
void hhconsother::on_BitBtn12_clicked()
{
...
void hhconsother::afterApply(QList<TmoduleInsertedKeys> insertedKeys)
{
    ...
    for (pos =0; pos <= insertedKeys.count()-1;pos++)
    {
        ...
        {
            sql = "INSERT INTO otherhhcpperiods (";
...
void hhconsother::loadProdCalc (const QModelIndex &index)
{
    bool rowOk;
    bool colOk;
    rowOk = true;
...
void hhconsother::loadDetCalc (const QModelIndex &index)
{
    bool rowOk;
    bool colOk;
    rowOk = false;
...
void hhconsother::on_pushButton_clicked()
{
    if (ui->AdvListView1->currentIndex().isValid())
    {
        hhconsprodslkp hhdialog;
        hhdialog.setDatabase(dataBase);
        hhdialog.loadData();
        hhdialog.setWindowTitle(tr("List of household consumption products"));
        hhdialog.exec();
        if (hhdialog.selected)
        {
            QModelIndex idx;
            idx = ui->AdvListView1->currentIndex();
            QModelIndex idx2;
            idx2 = m_colModel->index(idx.row(),2);
            m_colModel->setData(idx2,QString::number(hhdialog.energy,'f',2));
            idx2 = m_colModel->index(idx.row(),3);
            m_colModel->setData(idx2,QString::number(hhdialog.protein,'f',2));
        }
    }
}
