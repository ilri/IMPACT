#include "cropharvest.h"
#include "ui_cropharvest.h"
...
cropManagementModel::cropManagementModel(QObject *parent)
    :managementModel(parent)
{

}

void cropManagementModel::beforeInsert(QString table, QList<TmoduleFieldDef> keyFields)
{
    if ((table.toLower() == "crpprodman") || (table.toLower() == "crpforaman"))
    {
        QString sql;
        QSqlQuery mtable(database);
        int pos;
        sql = "INSERT INTO cropharvest (";
...
cropharvest::cropharvest(QWidget *parent,QSqlDatabase db, QString systemID) :
    QWidget(parent),
    ui(new Ui::cropharvest)
{
    dataBase = db;
...
{
    //qDebug() << "Before destroy cropharvest UI";
    delete ui;
    //qDebug() << "After destroy cropharvest UI";
}

void cropharvest::loadAndroidEffects()
{
    cropcharm.activateOn(ui->AdvListView1);
    actcharm.activateOn(ui->AdvListView5);
}

void cropharvest::loadForm()
{
    ...
    m_harvestModel = new cropManagementModel(this);
    ...
    m_harvestModel->addMasterTable("crpprodman");
    m_harvestModel->setKeysFields("crpprodman",cpmankeys);
    m_harvestModel->setLookUpLinkedField("crpprodman","PROD_COD");
    m_harvestModel->setPeriodTable("crpprodman","crpprodmantim");
    key.code = "PROD_COD";
    cpmankeys.append(key);
    m_harvestModel->setPeriodKeyFields("crpprodman",cpmankeys);
    m_harvestModel->setPeriodField("crpprodman","NMONTH");
    m_harvestModel->setPeriodStatusField("crpprodman","CHECKED");
    m_harvestModel->setPeriodDefaultValue("crpprodman","N");

    // We don't use a standard lookup table as the products change
    // between crops
    m_harvestModel->setlkUpUnChkColor("crpprodman",QColor(255,222,222));
    m_harvestModel->setlkUpChkColor("crpprodman",QColor(255,191,191));
    m_harvestModel->setlkUpColOrder("crpprodman","1P");
    cpmankeys.removeLast();
    //Add the harvesting of forages
    m_harvestModel->addMasterTable("crpforaman");
    m_harvestModel->setKeysFields("crpforaman",cpmankeys);
    m_harvestModel->setLookUpLinkedField("crpforaman","FORA_COD");
    m_harvestModel->setPeriodTable("crpforaman","crpforamantim");

    key.code = "FORA_COD";
    cpmankeys.append(key);
    m_harvestModel->setPeriodKeyFields("crpforaman",cpmankeys);
    m_harvestModel->setPeriodField("crpforaman","NMONTH");
    m_harvestModel->setPeriodStatusField("crpforaman","CHECKED");
    m_harvestModel->setPeriodDefaultValue("crpforaman","N");

    // We don't use a standard lookup table as the products change
    // between crops
    m_harvestModel->setlkUpUnChkColor("crpforaman",QColor(222,255,222));
    m_harvestModel->setlkUpChkColor("crpforaman",QColor(191,255,191));
    m_harvestModel->setlkUpColOrder("crpforaman","2F");
...
void cropharvest::constructCustomDelegator()
{
...
void cropharvest::constructHeader1(QList<TmoduleFieldDef> keys)
{
    header1 = "";
    QString plot = "";
    QString season = "";
...
void cropharvest::parentViewActivated(const QModelIndex &index)
{
    QList<TmoduleFieldDef> keys;
...
void cropharvest::harvestingChanged()
{
...
void cropharvest::on_BitBtn5_clicked()
{
    if (appyHarvestData)
    {
        QList<TmoduleFieldDef> keys;
...
void cropharvest::on_BitBtn1_clicked()
{
    if (appyHarvestData)
    {
...
void cropharvest::loadProdandFora(QString crop)
{
    ...
    while(lktable.next())
    {
        m_harvestModel->insertItem("crpprodman",lktable.value(0).toString(),
    ...
    while(lktable.next())
    {
        m_harvestModel->insertItem("crpforaman",lktable.value(0).toString(),
                                   lktable.value(1).toString());
    }

}

void cropharvest::constructPorFPopUpMenu(QPoint mousepos, QModelIndex index)
{
    QModelIndex idx;
...
void cropharvest::cropMenuRequested(QPoint mousepos)
{
    QModelIndex pos;
...
void cropharvest::createActions()
{
...
//void cropharvest::beforeInsert(QString table, QList<TmoduleFieldDef> keyFields)
//{
    //
//}

void cropharvest::on_cmddetails_clicked()
{
