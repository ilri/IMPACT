...
vegman::vegman(QWidget *parent,QSqlDatabase db, QString systemID) :
    QWidget(parent),
    ui(new Ui::vegman)
{
    dataBase = db;
...
void vegman::loadForm()
{
    ...
    m_activityModel->addMasterTable("vegman");
    m_activityModel->setKeysFields("vegman",cpmankeys);
    m_activityModel->setLookUpLinkedField("vegman","ACTY_COD");
    m_activityModel->setPeriodTable("vegman","vegmantim");
    key.code = "ACTY_COD";
    cpmankeys.append(key);
    m_activityModel->setPeriodKeyFields("vegman",cpmankeys);
    m_activityModel->setPeriodField("vegman","NMONTH");
    m_activityModel->setPeriodStatusField("vegman","CHECKED");
    m_activityModel->setPeriodDefaultValue("vegman","N");



    m_activityModel->setlkUpTable("vegman","activity");
    m_activityModel->setlkUpCodeFields("vegman","ACTY_COD");
    m_activityModel->setlkUpDescFields("vegman","ACTY_DES");
    m_activityModel->setlkUpWhereClause("vegman"," ACTY_TYP = 'C'");
    m_activityModel->setlkUpColDescription("Activities");
    m_activityModel->setlkUpUnChkColor("vegman",QColor(230,255,255));
    m_activityModel->setlkUpChkColor("vegman",QColor(199,255,255));

    m_actSortModel->setSourceModel(m_activityModel);
    m_activityModel->loadItems();
    //m_activityModel->setItemEdtStatus("vegman","006",false);
...
void vegman::constructCustomDelegator()
{
...
void vegman::constructHeader1(QList<TmoduleFieldDef> keys)
{
    header1 = "";
    QString plot = "";
    QString season = "";
...
void vegman::parentViewActivated(const QModelIndex &index)
{
    QList<TmoduleFieldDef> keys;
...
void vegman::on_BitBtn5_clicked()
{
    if (applyAcyData)
    {
...
void vegman::on_BitBtn1_clicked()
{
    if (applyAcyData)
    {
...
void vegman::constructActPopUpMenu(QPoint mousepos, QModelIndex index)
{
    QModelIndex idx;
    ...
    else
    {
            ...
            (m_activityModel->getCheckedStatusForRow(idx.row()) == true))
        {
            ...
            mlabourTable = "veglabour";
            menu.addAction(showLabourAct);
            if (isActyType(subModuleKeyFields,"F"))
            {
                mfertTable = "vegmanfert";
                manureTable = "vegmanheap";
...
void vegman::cropMenuRequested(QPoint mousepos)
{
    QModelIndex pos;
...
void vegman::actMenuRequested(QPoint mousepos)
{
    //qWarning() << "In here!";
    QModelIndex pos;
...
void vegman::loadFerts() //fertiliserallocation
{
...
bool vegman::isActyType(QList<TmoduleFieldDef> keys, QString type)
{
    QString actyValue;
...
void vegman::on_cmddetails_clicked()
{
