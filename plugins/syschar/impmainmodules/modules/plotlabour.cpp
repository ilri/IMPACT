#include "plotlabour.h"
#include "ui_plotlabour.h"
...
plotlabour::plotlabour(QWidget *parent,QSqlDatabase db, QString systemID) :
    QWidget(parent),
    ui(new Ui::plotlabour)
{
    dataBase = db;
...
void plotlabour::loadForm()
{
    ...
    m_parentModel->setTableName("spadstown");

    m_parentModel->insertField("SYS_COD","Sys ID","000","000",true,false,false);
    m_parentModel->insertField("PLOT_COD","Plot","000","000",true,false,true,false);
    m_parentModel->insertField("PLOT_ID","ID","000","000",false,false,true,false);
    m_parentModel->insertField("STATUS","Ownership","000","000",false,false,true,false);
    m_parentModel->insertField("TYPE","Usage","000","000",false,false,true,false);
    ...
    m_activityModel->addMasterTable("plotman");
    m_activityModel->setKeysFields("plotman",cpmankeys);
    m_activityModel->setLookUpLinkedField("plotman","ACTY_COD");
    m_activityModel->setPeriodTable("plotman","plotmantim");
    key.code = "ACTY_COD";
    cpmankeys.append(key);
    m_activityModel->setPeriodKeyFields("plotman",cpmankeys);
    m_activityModel->setPeriodField("plotman","NMONTH");
    m_activityModel->setPeriodStatusField("plotman","CHECKED");
    m_activityModel->setPeriodDefaultValue("plotman","N");



    m_activityModel->setlkUpTable("plotman","activity");
    m_activityModel->setlkUpCodeFields("plotman","ACTY_COD");
    m_activityModel->setlkUpDescFields("plotman","ACTY_DES");
    m_activityModel->setlkUpWhereClause("plotman"," ACTY_TYP = 'C' or ACTY_TYP = 'O'");
    m_activityModel->setlkUpColDescription("Activities");
    m_activityModel->setlkUpUnChkColor("plotman",QColor(230,255,255));
    m_activityModel->setlkUpChkColor("plotman",QColor(199,255,255));
...
void plotlabour::constructCustomDelegator()
{
    ...
    ui->AdvListView1->setItemDelegateForColumn(2,pdelegate);
...
void plotlabour::constructHeader1(QList<TmoduleFieldDef> /*keys*/)
{
    header1 = "";
}

void plotlabour::parentViewActivated(const QModelIndex &index)
{
    QList<TmoduleFieldDef> keys;
...
void plotlabour::activityChanged()
{
...
void plotlabour::on_BitBtn5_clicked()
{
    if (applyAcyData)
    {
...
void plotlabour::on_BitBtn1_clicked()
{
    if (applyAcyData)
    {
...
bool plotlabour::isActyType(QList<TmoduleFieldDef> keys, QString type)
{
    QString actyValue;
...
void plotlabour::constructActPopUpMenu(QPoint mousepos, QModelIndex index)
{
    QModelIndex idx;
    ...
    else
    {
            ...
            (m_activityModel->getCheckedStatusForRow(idx.row()) == true))
        {
            ...
            mlabourTable = "plotlabour";
            menu.addAction(showLabourAct);
            usePlantingDensity = false;
            if (isActyType(subModuleKeyFields,"F"))
            {
                mfertTable = "plotmanfert";
                manureTable = "plotmanheap";
...
void plotlabour::cropMenuRequested(QPoint /*mousepos*/)
{
    /*QModelIndex pos;
...
void plotlabour::actMenuRequested(QPoint mousepos)
{
    //qWarning() << "In here!";
    QModelIndex pos;
...
void plotlabour::loadFerts() //fertiliserallocation
{
...
void plotlabour::on_cmddetails_clicked()
{
